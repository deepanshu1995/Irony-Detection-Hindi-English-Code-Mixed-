...
internet_laughs = \
	['lol', 'Haha', 'ha', 'ha!', 'HAHAHAHAHAHHA', 'hehehe', \
	'lolololol', 'Crying', 'Sobbing', 'Dying', 'Dead', 'LMAO', \
	'LMFAO', 'LOL', 'lel', 'lawl', 'lollll', 'lololol', 'rotfl', \
	'rotflol', 'Lollerskates', 'lollercoaster', 'loltastic', 'roflcopter', \
	'lulz', 'behehe', 'ahehahe', 'abaha', 'BAHAHA', 'mwahaha', 'muahahah', 
	]
...
def CountOfWordsInATweet(tweet):
	count = 0
	if 'URL' in tweet:
		count = count + 1
	if 'EMOTICON' in tweet:
	...
	word_count = len(tweet) - count
	return word_count

def CountOfCharsInATweet(tweet):
	count = 0
	for token in tweet:
		if token!='URL' and token!='EMOTICON' and token!='USER' and token!='HASHTAG':
...
def FindInternetLaughs(tweet):
	laugh_words = []
	for laugh_word in internet_laughs:
		if laugh_word in tweet:
			laugh_words.append(laugh_word)
...
def PreProcessing(tweet):
	...
	word_count = CountOfWordsInATweet(processed_tweet_tokenised)
	char_count = CountOfCharsInATweet(processed_tweet_tokenised)
	laugh_words = FindInternetLaughs(processed_tweet_tokenised)
	word_length_mean = char_count/word_count
